...
public class GaussRightMethodSolver extends InterpolationSolver {
    @Override
    ...
    protected Function<Double, Double> calcPolynomial(Dot[] dots) {
        ...
        return (Double x) -> {
            int lT = 0;
            int rT = 0;
            int iterFD = dots.length/2;
            double t = (x - dots[dots.length/2].x())/h;
            double sum = finiteDifferences[iterFD][0] + t * finiteDifferences[iterFD][1];
